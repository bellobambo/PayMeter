...
export default function ConsoleIntegrationPage() {
  return (
    <>
      <ConsoleHeader
        description="A judge-friendly view of how Studio, CaptionPilot, and the payment rail stay separated while still sharing one usage-based billing flow."
        eyebrow="Integration"
      ...
      <section className="mt-6 grid gap-4 md:grid-cols-2">
        <div className="rounded-lg border border-ink/10 bg-white p-5 shadow-line">
          ...
          <p className="mt-2 text-sm leading-6 text-graphite">
            {getApiBaseUrl() || "No backend URL is configured, so the same product flow runs with preview data."}
          </p>
        </div>
